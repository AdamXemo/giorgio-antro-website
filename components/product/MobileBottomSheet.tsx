...
export default function MobileBottomSheet({ product, addedToCart, onAddToCart }: MobileBottomSheetProps) {
  ...
  return (
    <div
      ref={sheetRef}
      className="fixed bottom-0 left-0 right-0 z-20 h-[82dvh] flex flex-col bg-white rounded-t-2xl shadow-[0_-8px_30px_rgba(0,0,0,0.08)]"
      style={{
    ...
    >
      ...
      >
        <div className="flex justify-center mb-3">
          <div className="w-10 h-[3px] rounded-full bg-black/[0.12]" />
        </div>
        <h1 className="font-display font-light text-xl leading-tight tracking-tight">
          {product.name}
        </h1>
        <p className="text-sm font-light tabular-nums mt-1 text-black/60">
        ...
        <div className="pt-6 border-t border-black/[0.07]">
          ...
          <div>
            ...
            <button
              ...
              aria-pressed="true"
              className="w-6 h-6 rounded-full bg-black ring-1 ring-offset-2 ring-black"
            />
          </div>
