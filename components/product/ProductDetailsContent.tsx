...
export default function ProductDetailsContent({ type, product, variant }: ProductDetailsContentProps) {
  if (type === 'details') {
    return variant === 'drawer' ? (
      <>
        <p className="font-body text-[13px] text-black/55 leading-loose mb-10">
          {product.description}
        </p>
        <div className="border-t border-black/[0.07]">
          {product.features.map((feature, idx) => (
            ...
            >
              ...
              <span className="text-[13px] text-black/65">{feature}</span>
            </div>
          ))}
        </div>
      </>
      ...
      <div className="pb-6">
        <p className="font-body text-sm text-black/55 leading-loose mb-5">
          {product.description}
        </p>
        {product.features.map((feature, idx) => (
          <div
            key={idx}
            className="flex items-center gap-5 py-3 border-b border-black/[0.05] last:border-0"
          >
            <span className="text-[10px] text-black/25 tabular-nums w-5 flex-shrink-0">
              {String(idx + 1).padStart(2, '0')}
            </span>
  ...
  return variant === 'drawer' ? (
    <>
      <p className="text-[13px] text-black/55 leading-loose mb-10">
        {product.shippingInfo}
      </p>
      <div className="pt-8 border-t border-black/[0.07]">
        <p className="text-[9px] tracking-[0.25em] mb-5">RETURNS</p>
        <p className="text-[13px] text-black/55 leading-loose">
    ...
    <div className="pb-6 space-y-4 text-sm text-black/55 leading-loose">
